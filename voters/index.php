								...
								<?php
									...
									?>
										<tr>
											...
											<td>
												<?php
													$checkIfVoteCasted = mysqli_query($db, "SELECT * FROM votings WHERE voter_id = '".$_SESSION['user_id']."' AND election_id = '".$election_id."' ") or die(mysqli_error($db));
													...
													if($isVoteCasted > 0)
													{
														...
														{
															?>
															<button class="btn btn-md btn-success disabled">Voted</button>
														<?php
													}
													} else
													{
